...
export const createDataStorePredicate = (predicateObject) => {
  ...
  return (p) => {
    // Validate field name to prevent arbitrary property access
    const isValidFieldName = (fieldName) => {
      // Only allow alphanumeric characters, underscores, and dots for nested fields
      return typeof fieldName === 'string' && /^[a-zA-Z_][a-zA-Z0-9_.]*$/.test(fieldName);
    };
    
    // Safe operator execution without dynamic property access
    const executeOperator = (predicate, fieldName, operatorName, operand) => {
      if (!isValidFieldName(fieldName)) {
        return predicate;
      }
      
      const fieldPredicate = predicate[fieldName];
      if (!fieldPredicate || typeof fieldPredicate !== 'object') {
        return predicate;
      }
      ...
      switch (operatorName) {
        case 'eq':
          return fieldPredicate.eq ? fieldPredicate.eq(operand) : predicate;
        case 'ne':
          return fieldPredicate.ne ? fieldPredicate.ne(operand) : predicate;
        case 'gt':
          return fieldPredicate.gt ? fieldPredicate.gt(operand) : predicate;
        case 'ge':
          return fieldPredicate.ge ? fieldPredicate.ge(operand) : predicate;
        case 'lt':
          return fieldPredicate.lt ? fieldPredicate.lt(operand) : predicate;
        case 'le':
          return fieldPredicate.le ? fieldPredicate.le(operand) : predicate;
        case 'contains':
          return fieldPredicate.contains ? fieldPredicate.contains(operand) : predicate;
        case 'notContains':
          return fieldPredicate.notContains ? fieldPredicate.notContains(operand) : predicate;
        case 'beginsWith':
          return fieldPredicate.beginsWith ? fieldPredicate.beginsWith(operand) : predicate;
        case 'between':
          return fieldPredicate.between ? fieldPredicate.between(operand) : predicate;
        case 'attributeExists':
          return fieldPredicate.attributeExists ? fieldPredicate.attributeExists(operand) : predicate;
        case 'attributeType':
          return fieldPredicate.attributeType ? fieldPredicate.attributeType(operand) : predicate;
        case 'size':
          return fieldPredicate.size ? fieldPredicate.size(operand) : predicate;
        default:
          return predicate;
      }
    };
    ...
    if (!!field && !!operator) {
      return executeOperator(p, field, operator, operand);
    }
    return p;
  };
};
...
const checkValidation = (value, validation) => {
  ...
  switch (validation.type) {
    case "Email":
      // Safe email validation to prevent ReDoS attacks
      const isValidEmail = (email) => {
        // Basic format check without complex regex
        ...
        // Simple character validation for local part
        ...
        // Simple character validation for domain part
    ...
    case "IpAddress":
      // Safe IP address validation to prevent ReDoS attacks
      ...
      const isValidIPv6 = (ip) => {
        // Simple IPv6 validation without complex regex to prevent ReDoS
        if (ip.includes('::')) {
          const parts = ip.split('::');
          if (parts.length > 2) return false;
          // Basic validation for compressed IPv6
          return parts.every(part => {
            if (!part) return true;
      ...
      const isValidIP = isValidIPv4(value) || isValidIPv6(value);
      return {
        hasError: !isValidIP,
        errorMessage:
          validation.validationMessage ||
